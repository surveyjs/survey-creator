...
import { IJournalArrayChangedPayload, IJournalElementMovedPayload, IJournalPropertyChangedPayload, JOURNAL_VERSION, JournalOp, JournalSyncError } from "../src/plugins/collaboration/journal/journal-record";
...
function createPair(json: any = initialJSON): { a: CreatorTester, pluginA: CollaborationPlugin, b: CreatorTester, pluginB: CollaborationPlugin, sync: () => void, expectInSync: () => void } {
  ...
  b.addPlugin("collaboration", pluginB);
  const sync = (): void => {
...
test("journal: coalescing disabled with coalesceInterval 0", (): any => {
  const a = new CreatorTester();
  a.JSON = initialJSON;
  const pluginA = new CollaborationPlugin(a, { coalesceInterval: 0, presence: false, bar: false });
  a.addPlugin("collaboration", pluginA);
  const q = a.survey.getQuestionByName("q1");
  q.title = "t";
  q.title = "ti";
...
test("journal: concurrent edits of different locales of the same string converge", (): any => {
  ...
  const makeOne = () => {
    ...
    const p = new CollaborationPlugin(c, { presence: false, bar: false });
    c.addPlugin("collaboration", p);
...
test("journal: constructor throws without the undo-redo plugin", (): any => {
  ...
  // Deliberately WITHOUT presence:false/bar:false - asking for the full set is
  // what makes this also a test of the construction order. The journal is built
  // first precisely so that its throw leaves nothing behind.
  const layersBefore = document.body.querySelectorAll(".collab-presence-layer").length;
  expect(() => new CollaborationPlugin(creator)).toThrow(/undo-redo/);
  expect(creator.collabBar).toBeFalsy();
  expect(document.body.querySelectorAll(".collab-presence-layer").length).toEqual(layersBefore);
});

test("journal: dispose releases the undo-redo validity callback", (): any => {
  const creator = new CreatorTester();
  creator.JSON = initialJSON;
  const plugin = new CollaborationPlugin(creator, { presence: false, bar: false });
  creator.addPlugin("collaboration", plugin);
  expect(creator.undoRedoManager.isTransactionValidCallback).toBeTruthy();

  plugin.dispose();
  // Otherwise the guard - and its WeakSet of removed objects - stays reachable
  // from the live manager, and every undo goes on consulting a dead guard.
  expect(creator.undoRedoManager.isTransactionValidCallback).toBeFalsy();
});

test("journal: dispose does not steal a validity callback installed by someone else", (): any => {
  const creator = new CreatorTester();
  creator.JSON = initialJSON;
  const plugin = new CollaborationPlugin(creator, { presence: false, bar: false });
  creator.addPlugin("collaboration", plugin);
  ...
  creator.undoRedoManager.isTransactionValidCallback = foreign;
  plugin.dispose();
  expect(creator.undoRedoManager.isTransactionValidCallback).toBe(foreign);
});
