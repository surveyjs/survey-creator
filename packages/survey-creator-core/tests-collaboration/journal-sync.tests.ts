...
// test suite (feature/undo-redo-sync branch) on top of the CollaborationPlugin.
...
function wire(from: CollaborationPlugin, to: CollaborationPlugin): void {
...
function makeCreators(json: any = { pages: [{ name: "page1" }] }, options: any = {}) {
  ...
  creatorA.JSON = json;
  creatorB.JSON = json;
  const pluginA = new CollaborationPlugin(creatorA, { presence: false, bar: false });
  creatorA.addPlugin("collaboration", pluginA);
  const pluginB = new CollaborationPlugin(creatorB, { presence: false, bar: false });
  creatorB.addPlugin("collaboration", pluginB);
