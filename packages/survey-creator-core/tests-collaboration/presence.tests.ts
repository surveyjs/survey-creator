...
function createCreator(): { creator: CreatorTester, plugin: CollaborationPlugin, states: Array<IPresenceState> } {
...
test("presence: designer name badge hides under the flyout panel, pg badge does not", (): any => {
  ...
  document.body.appendChild(sidebar);
  mockRect(sidebar.querySelector(".svc-side-bar__container"), 550, 0, 300, 700);
  const badgeEl = (): HTMLElement => (<any>plugin.presence.overlay).layer.querySelector(".collab-presence-badge");
  try {
    plugin.upsertPeer(peerEntry("c1", {
      state: {
        tab: "designer",
...
function createTranslationCreator(): { creator: CreatorTester, plugin: CollaborationPlugin, matrix: any, stringsSurvey: any } {
