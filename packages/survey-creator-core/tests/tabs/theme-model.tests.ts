...
test("Theme model de/serialization", (): any => {
  ...
  const result = themeEditor.toJSON();
  expect(result.cssVariables).toStrictEqual(themeJson.cssVariables);
  expect(result.cssVariables["--sjs-base-unit"]).toBe("6px");
  expect(result.cssVariables["--sjs-corner-radius"]).toBe("20px");
  expect(result.cssVariables["--sjs-font-size"]).toBe("17.6px");
});
...
test("Theme builder: composite question fontSettings", (): any => {
  const themeEditor = new ThemeModel();
  const questionTitleFontSettings = themeEditor.questionTitle;

  expect(themeEditor["--sjs-font-questiontitle-family"]).toBeUndefined();
  expect(themeEditor["--sjs-font-questiontitle-weight"]).toBeUndefined();
  expect(themeEditor["--sjs-font-questiontitle-color"]).toBeUndefined();
  expect(themeEditor["--sjs-font-questiontitle-size"]).toBeUndefined();
  ...
  expect(themeEditor["--sjs-font-questiontitle-family"]).toEqual("Arial, sans-serif");
  expect(themeEditor["--sjs-font-questiontitle-weight"]).toEqual("500");
  expect(themeEditor["--sjs-font-questiontitle-color"]).toBeUndefined();
  expect(themeEditor["--sjs-font-questiontitle-size"]).toEqual("40px");
