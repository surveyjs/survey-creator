...
test("Several pages for one category", () => {
  ...
  options.showOneCategoryInPropertyGrid = true;
...
test("selectProperty(propName, false) ", () => {
  ...
  const options = new EmptySurveyCreatorOptions();
  options.showOneCategoryInPropertyGrid = true;
  const propertyGrid = new PropertyGridModelTester(dropdown, options);
  const propSurvey = propertyGrid.survey;
...
test("Go to visibleIf link", () => {
  ...
  const options = new EmptySurveyCreatorOptions();
  options.showOneCategoryInPropertyGrid = true;
  let propertyGrid = new PropertyGridModelTester(question, options);
