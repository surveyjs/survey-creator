import { Base, SurveyModel, surveyLocalization } from "survey-core";
import type { SurveyCreatorModel } from "../creator-base";
import type { ICreatorPlugin } from "../creator-settings";
import {
  IUndoRedoAction,
  IUndoRedoChange,
  Transaction,
  ...
  UndoRedoAction,
  UndoRedoArrayAction
} from "../plugins/undo-redo/undo-redo-manager";
import {
  applyAction,
  buildPropertyAction,
  captureInverse,
...
import {
  planTranslationRefresh,
  applyTranslationRefresh,
  planLogicRefresh,
  applyLogicRefresh,
  bindLogicEditEndFlush
} from "./tab-refresh-planners";
import { RemoteBulkAction, RemoteUndoRedoAction } from "./remote-actions";
import { TabSyncRefresher } from "./tab-sync-refresher";
...
export class UndoRedoSyncPlugin implements ICreatorPlugin {
  ...
  // Saved hooks for `dispose` (assigned in `attach`, invoked from the ctor).
  ...
  // Refreshes the Translation/Logic tab snapshot models after a remote message
  // is applied. Only created for a full creator (one exposing `getPlugin`); the
  // headless server's duck-typed creator has no tabs, so it stays undefined.
  private tabRefresher?: TabSyncRefresher;
  ...
  constructor(private creator: SurveyCreatorModel, transport?: ISyncTransport) {
    ...
    this.attach();
    // Feature-detect a full creator: the headless server passes a duck-typed
    // { undoRedoManager, survey } object with no `getPlugin`, so it gets no
    // tab refresher and `applySerialized` stays a pure model mutation there.
    if (typeof (creator as any).getPlugin === "function") {
      this.tabRefresher = new TabSyncRefresher(creator as any);
    }
  ...
  public dispose(): void {
    this.detach();
    this.tabRefresher?.dispose();
    this.tabRefresher = undefined;
  ...
  public applySerialized(message: ISyncMessage): void {
    ...
    // The live survey is now mutated and its observer restored. Refresh the
    // active Translation/Logic tab snapshot model so it reflects the change
    // (no-op on other tabs, and skipped entirely on the headless server).
    this.tabRefresher?.refresh(message);
  }
