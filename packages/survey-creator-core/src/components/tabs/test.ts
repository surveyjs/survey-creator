...
export class TestSurveyTabViewModel extends Base {
  static tagRegex = /(<([^>]+)>)/ig;
  public static enableInvisiblePages: boolean = true;
  private json: any;
  ...
  private updatePageItem(page: PageModel) {
    ...
    if (item) {
      item.enabled = TestSurveyTabViewModel.enableInvisiblePages || page.isVisible;
    }
  }
  ...
  private updatePageList() {
    ...
    for (let i: number = 0; i < this.survey.pages.length; i++) {
      ...
      pages.push({
        ...
        enabled: TestSurveyTabViewModel.enableInvisiblePages || page.isVisible,
        visible: true
      });
    }
