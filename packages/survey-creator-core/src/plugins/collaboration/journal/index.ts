...
// Records all survey modifications as a serializable action journal and can
// apply such a journal to synchronize this creator with another one (via a
// file, websocket, etc.).
//
...
export class JournalController {
  ...
  public dispose(): void {
    ...
    // Without this the guard - and the WeakSet of removed objects it carries -
    // stays reachable from the creator's undo-redo manager forever.
    this.guard.detach();
  }
