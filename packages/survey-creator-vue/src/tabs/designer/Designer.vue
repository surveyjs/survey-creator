<template>
  ...
  <div class="svc-tab-designer" :class="model.getRootCss()" data-bind="css: getRootCss(), click: clickDesigner">
    <div class="svc-tab-designer_content">
      <template v-if="model.showPlaceholder">
        <div v-if="model.creator.showHeaderInEmptySurvey && model.creator.allowEditSurveyTitle" class="svc-designer-header">
        ...
        <div class="svc-designer__placeholder-container"
          ...
          <svc-page class="svc-designer-placeholder-page" :survey="model.creator.survey" :creator="model.creator" :page="model.newPage">
          </svc-page>
        </div>
      </template>

      <template v-if="!model.showPlaceholder">
        <div :class="model.designerCss" data-bind="css: designerCss, style:{maxWidth: survey.renderedWidth}">
          ...
          <template v-if="model.creator.pageEditMode !== 'bypage'">
            <svc-page v-for="(page, index) in model.creator.survey.pages" class="svc-page"
              data-bind="attr: { 'data-sv-drop-target-survey-element': $data.name, 'data-sv-drop-target-page': $data.name }"
              :survey="model.creator.survey" :creator="model.creator" :page="page"></svc-page>
            <svc-page v-if="model.showNewPage" class="svc-page" data-bind="attr: { 'data-sv-drop-target-survey-element': 'newGhostPage' }"
              :survey="model.creator.survey" :creator="model.creator" :page="model.newPage"></svc-page>
          </template>

          <svc-page v-if="model.pagesController.page2Display && model.creator.pageEditMode === 'bypage'" class="svc-page"
        ...
        <div v-if="model.creator.showPageNavigator" class="svc-tab-designer__page-navigator">
          <svc-page-navigator :controller="model.pagesController" :pageEditMode="model.creator.pageEditMode">
          </svc-page-navigator>
        </div>
      </template>
    </div>
