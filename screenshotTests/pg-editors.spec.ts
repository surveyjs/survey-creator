...
import { url, setJSON, getPropertyGridCategory, generalGroupName, addQuestionByAddQuestionButton, resetHoverToCreator, surveySettingsButtonSelector, inputMaskSettingsGroupName, getListItemByText, getToolboxItemByText, getQuestionBarItemByTitle, setShowToolbox, setShowAddQuestionButton, setAllowEditSurveyTitle, getAddNewQuestionButton, compareScreenshot, doDragDrop, setIsCompact } from "./helper";
...
test("Default value popup", async ({ page }) => {
  ...
  await getToolboxItemByText(page, "Single-Line Input").hover({ position: { x: 25, y: 0 } });
  await getToolboxItemByText(page, "Single-Line Input").click({ position: { x: 25, y: 0 } });
  await getPropertyGridCategory(page, generalGroupName).click();
  await getPropertyGridCategory(page, "Data").click();
...
test("Custom button into fast entry popup", async ({ page }) => {
  ...
  await page.evaluate(() => {
    (<any>window).creator.onPropertyGridShowModal.add((sender, options) => {
      const editor = options.popUP_editor;
      options.popUP_Model.footerToolbar.addAction({
...
test("Check color editor", async ({ page }) => {
  ...
  await resetHoverToCreator(page);
  await compareScreenshot(page, page.locator(".sv-popup__container").filter({ hasText: "" }).first(), "color-editor-choices.png");
...
test("Check property grid panel' header states", async ({ page }) => {
  await setJSON(page, {});
  await page.setViewportSize({ width: 1920, height: 1920 });
  const headerSelector = getPropertyGridCategory(page, generalGroupName).first();
...
test("Dropdown input in property grid", async ({ page }) => {
  ...
  await page.keyboard.type("a l i");
...
test("Property grid checkbox with description", async ({ page }) => {
  ...
  await compareScreenshot(page, page.locator("[data-name='visible']"), "pg-checkbox-caption-opened.png");
});
...
test("popup overlay in property grid", async ({ page }) => {
  ...
  await compareScreenshot(page, page.locator(".sv-popup").filter({ hasText: "" }).first(), "pg-overlay-popup.png");
});
...
test("Check creator theme settings", async ({ page }) => {
  ...
  await compareScreenshot(page, page.locator(".spg-body").filter({ hasText: "" }).first(), "creator-theme-settings.png");
});

test("Helper action not hidden", async ({ page }) => {
  await page.setViewportSize({ width: 1240, height: 870 });
  const westResizer = page.locator(".svc-resizer-west");
  const questionHeader = getPropertyGridCategory(page, "Conditions").locator("..").locator(".spg-panel").locator(".spg-question__header");
