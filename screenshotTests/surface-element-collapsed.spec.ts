...
import { url, compareScreenshot, test, setJSON, setShowSidebar, setShowAddQuestionButton, setAllowEditSurveyTitle, setExpandCollapseButtonVisibility, resetHoverToCreator, setShowToolbox, setAllowZoom } from "./helper";
...
test("Question adorner - collapsed", async ({ page }) => {
  ...
  const qCollapseButton = page.getByRole("button", { name: "Collapse", exact: true }).first();
  ...
  await qCollapseButton.nth(2).click();
