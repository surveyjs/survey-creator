...
QUnit.test("toolbox support options", function (assert) {
    ...
    assert.equal(toolbox.items[0].json["type"], toolbox.orderedQuestions[0], "the type is 'text'");
    var toolbox = new QuestionToolbox(["text", "dropdown", "unknown"]);
    assert.equal(toolbox.items.length, 2, "Only two types from three are accepted");
});
...
QUnit.test("toolbox copied questions", function (assert) {
    ...
    assert.equal(toolbox.copiedItems.length, 3, "remove the first copied item on adding");
    assert.equal(toolbox.items.length, 5, "There are 5 items total");
