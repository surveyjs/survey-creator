...
test("Question type popup - wide", async (t) => {
  ...
  await t
    ...
    .expect(Selector(".sv-popup:not(.sv-popup--overlay) li").withText("Text Entry").visible).ok();
});
...
test("Question type popup - narrow", async (t) => {
  ...
  await t
    .resizeWindow(380, 600)
    .click(Selector("button.svc-page__question-type-selector"))
    .expect(Selector(".sv-popup.sv-popup--overlay li").withText("Text Entry").visible).ok();
});
