...
test('Single input question wrapper actions', async (t) => {
    ...
    await t.expect(Selector('.svc-question__content--selected').find('input[aria-label=question1]').visible).ok();
    
    const controls = Selector('.svc-question__content-actions').find('.sv-action');
    await t.expect(controls.count).eql(5);

    const convertActionButton = controls.nth(0).find('button[title="Convert to"]');
    ...
    const deleteActionButton = controls.nth(4).find('button[title="Delete"]');
...
test.skip('Single input question wrapper action convert', async (t) => {
    ...
    await t.expect(Selector('.svc-question__content--selected').find('textarea[aria-label=question1]').visible).ok();
...
test('Single input question wrapper action duplicate', async (t) => {
    ...
    await t.expect(questions.nth(0).hasClass('svc-question__content--selected')).notOk();
    await t.expect(questions.nth(1).hasClass('svc-question__content--selected')).ok();
});
...
test('Single input question wrapper action change require', async (t) => {
    ...
    await t.expect(requiredActionRoot.hasClass('sv-action-bar-item--secondary')).notOk();

    await t.click(requiredActionButton);
    title = await questionTitle.innerText;
    await t.expect(title.replace(/\xa0/gi, ' ')).eql('1. question1\n *');
    await t.expect(requiredActionRoot.hasClass('sv-action-bar-item--secondary')).ok();
});
