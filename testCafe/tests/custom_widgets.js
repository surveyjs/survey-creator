...
export const init = ClientFunction(() => {
  ...
  var barRatingWidget = {
    ...
    afterRender: function(question, el) {
      var $el = $(el)
      var ratingTheme = question.ratingTheme
        ? question.ratingTheme
        : 'fontawesome-stars'
  ...
  var iCheckWidget = {
    ...
    afterRender: function(question, el) {
      var select = function() {
        $(el)
          .find('input[value=' + question.value + ']')
          .iCheck('check')
      }
      $(el)
        .find('input')
  ...
  editor.onCanShowProperty.add(function(sender, options) {
    if (options.obj.getType() == 'dropdown') {
      var forbiddenBarRatingProperties = [
        'choicesOrder',
        'choicesByUrl',
        'optionsCaption',
        'hasOther',
      ...
      if (canShow && !isBarRating)
        canShow = forbiddenDropDownProperties.indexOf(propName) < 0
      //hide properties for Bar Rating
      if (canShow && isBarRating)
        canShow = forbiddenBarRatingProperties.indexOf(propName) < 0
